#!/usr/bin/env bash
# =============================================================================
# install.sh — One-Click Setup & Installer for jev-figure-router
# =============================================================================
set -e
...
echo -e "${BLUE}=================================================================${NC}"
echo -e "${BLUE}   jev-figure-router — Automated Installation & Agent Setup      ${NC}"
echo -e "${BLUE}=================================================================${NC}"
...
echo -e "\n${YELLOW}[2/4] Installing core visualization dependencies...${NC}"
if [ "$1" == "--venv" ]; then
...
if [ -n "$OPENROUTER_API_KEY" ]; then
    ...
    echo -e "  To enable 300ms fast routing, please run:"
...
for AGENT_DIR in "${AGENT_DIRS[@]}"; do
    ...
    if [ -d "$PARENT_DIR" ]; then
        mkdir -p "$AGENT_DIR"
        TARGET_LINK="${AGENT_DIR}/figure-router"
...
echo -e "\n${GREEN}=================================================================${NC}"
echo -e "${GREEN}   Installation Complete! Ready to route and generate figures!   ${NC}"
echo -e "${GREEN}=================================================================${NC}"
echo -e "Quick Tests:"
echo -e "  1. Test fast routing : ${BLUE}python3 scripts/fast_route.py -q \"Nature 声子谱折线图\" --template${NC}"
echo -e "  2. Run plot demo     : ${BLUE}python3 examples/demo_scientific_plot.py${NC}"
echo -e "  3. Open architecture : ${BLUE}open examples/demo_architecture_diagram.html${NC}"
